  ...
  const text = await askText([
    { role: "system", content: FIREWALL_SYSTEM },
    {
      ...
      content:
        ...
        "   - MANDATORY INPUT PROJECTION IN FORWARD(X): Define `self.input_proj = nn.Linear(input_dim, d_model)` in `__init__` AND call `x = self.input_proj(x)` at the very start of `forward(x)` before passing `x` to `nn.TransformerEncoder(x)` whenever `input_dim != d_model`.\n" +
